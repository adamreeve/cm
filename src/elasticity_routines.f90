...
MODULE ELASTICITY_ROUTINES
  ...
  PUBLIC Elasticity_StrainInterpolateXi, Elasticity_DeformationGradInterpolateXi
...
999 CALL Errors("Elasticity_StrainInterpolateXi",err,error)
  ...
  !>Interpolates the given parameter set at a specified xi location for the specified element and
  !>derviative and returns double precision values for a field identified by an object.
  SUBROUTINE Elasticity_DeformationGradInterpolateXi(equationsSet,userElementNumber,xi,values,err,error,*)
    ...
    CALL Enters("Elasticity_DeformationGradInterpolateXi",err,error,*999)
    ...
    CASE(EQUATIONS_SET_LINEAR_ELASTICITY_TYPE)
      CALL FlagError("Not implemented.",err,error,*999)
    CASE(EQUATIONS_SET_FINITE_ELASTICITY_TYPE)
      CALL FiniteElasticity_DeformationGradInterpolateXi(equationsSet,userElementNumber,xi,values,err,error,*999)
    CASE DEFAULT
      CALL FlagError("Equations set type "//TRIM(NumberToVstring(equationsSet%type,"*",err,error))// &
        & " is not valid for an elasticity class equation.",err,error,*999)
    END SELECT

    CALL Exits("Elasticity_DeformationGradInterpolateXi")
    RETURN
999 CALL Errors("Elasticity_DeformationGradInterpolateXi",err,error)
    CALL Exits("Elasticity_DeformationGradInterpolateXi")
    RETURN 1
  END SUBROUTINE Elasticity_DeformationGradInterpolateXi
